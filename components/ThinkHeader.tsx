...
export default function ThinkHeader({ title, type, typeColor: color, tags, readTime, publishedOn, whyQuestion }: Props) {
  const headerRef = useRef<HTMLDivElement>(null!);

  return (
    <div ref={headerRef} className="think-main-header" style={{ borderBottom:"1px solid var(--border)", padding:"3rem 2rem 2.5rem" }}>
      <div style={{ maxWidth:"760px", margin:"0 auto" }}>
        ...
        <div style={{ display:"flex", alignItems:"center", gap:"0.75rem", marginBottom:"1.5rem", flexWrap:"wrap" }}>
          <span style={{ fontFamily:S.mono, fontSize:"10px", letterSpacing:"0.12em", textTransform:"uppercase", color, border:`1px solid ${color}`, padding:"0.2rem 0.75rem" }}>{type}</span>
          {tags.map(t=>(
        ...
        {/* Title — sticky + shrinks on scroll */}
        <StickyTitle title={title} headerRef={headerRef} />
