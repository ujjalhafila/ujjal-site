...
const M = { mono:"'DM Mono',monospace" };
...
function GalleryCard({ src, label, onClick }: { src: string; label: string; onClick: () => void }) {
  ...
  const plat = type !== "image" ? getPlatform(src) : "";
    ...
    <button onClick={onClick} className="gallery-card" aria-label={`Open ${label}`}
      ...
      <div style={{ position:"relative", width:"100%", height:"200px", overflow:"hidden",
        ...
        <div className="gallery-overlay" style={{ position:"absolute", inset:0,
          ...
          {plat && <span style={{ fontFamily:M.mono, fontSize:"10px", letterSpacing:"0.1em",
            textTransform:"uppercase", color:"rgba(255,255,255,0.9)",
            textShadow:"0 1px 3px rgba(0,0,0,0.5)" }}>{plat}</span>}
        </div>
      </div>
...
// ── ClickableThumb — for work listing page ───────────────────────────────────
export function ClickableThumb({ src, alt, title }: { src: string; alt: string; title?: string }) {
  ...
  const open = useCallback(() => setModal({ src, alt: alt||title||"", type:"image" }), [src, alt, title]);
  const close = useCallback(() => setModal(null), []);

  return (
    <>
      <div onClick={open} className="clickable-thumb" role="button" tabIndex={0}
        ...
        <div className="thumb-zoom-hint" style={{ position:"absolute", bottom:"0.6rem",
          ...
          <span style={{ fontFamily:M.mono, fontSize:"9px", color:"white", letterSpacing:"0.08em", textTransform:"uppercase" }}>View</span>
        </div>
      </div>
...
export default function WorkGallery({ thumbnailUrl, videoDemo, title }: {
  ...
  const items = [
    ...
    videoDemo    ? { src: videoDemo,    label: isFigma(videoDemo) ? "Prototype" : "Video demo" } : null,
  ...
  return (
    <>
      <div style={{ display:"grid", gridTemplateColumns:items.length>1?"1fr 1fr":"1fr",
        gap:"1rem", marginBottom:"2.5rem" }} className="gallery-grid">
        {items.map((item, i) => (
