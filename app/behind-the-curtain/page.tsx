  ...
  return (
    ...
    >
      <div style={{
        ...
        <span style={{ fontFamily: MONO, fontSize: 10, color: "#1a0a2e", opacity: closing ? 0 : hover ? 0.55 : 0.3, transition: "opacity 0.2s", letterSpacing: "1.5px" }}>
...
function Marquee({ words, bg, color }:{ words:string[]; bg:string; color:string }) {
  ...
  return (
    <div style={{ overflow:"hidden", background:bg, padding:"12px 0", borderTop:`2px solid ${color}33`, borderBottom:`2px solid ${color}33` }}>
      <div style={{ display:"flex", whiteSpace:"nowrap", animation:"marquee 26s linear infinite", fontFamily:SANS, fontWeight:800, fontSize:"clamp(1.4rem,3.5vw,2.4rem)", letterSpacing:"-0.5px", color }}>
...
function WorkChip({ label, href, ink }:{ label:string; href:string; ink:string }) {
  ...
  const s: React.CSSProperties = {
    fontFamily:MONO, fontSize:11, textDecoration:"none", color:"#1a0a2e",
    background:"#FFE03A", border:"2px solid #1a0a2e", borderRadius:4,
    padding:"5px 14px 6px", display:"inline-flex", alignItems:"center", gap:6,
    boxShadow:"3px 3px 0 #1a0a2e", transition:"transform 0.15s, box-shadow 0.15s",
  };
  const inner = <>{label} <span style={{fontSize:12}}>↗</span></>;
  const on = (e:any) => { e.currentTarget.style.transform="translateY(-2px)"; e.currentTarget.style.boxShadow="4px 5px 0 #1a0a2e"; };
  const off = (e:any) => { e.currentTarget.style.transform=""; e.currentTarget.style.boxShadow="3px 3px 0 #1a0a2e"; };
...
function ExperimentCard({ ex }:{ ex:typeof EXPERIMENTS[0] }) {
  ...
  return (
    <button onClick={() => setOpen(o => !o)} style={{
      background:"#F7F3EC", border:"2.5px solid #1a0a2e", borderRadius:8,
      padding: open ? "28px 22px 22px" : "24px 20px", boxShadow: open ? "8px 8px 0 #1a0a2e" : "5px 5px 0 #1a0a2e",
      ...
      <span style={{ fontFamily:MONO, fontSize:9, letterSpacing:"1.2px", background:ex.color, color:"#1a0a2e", padding:"3px 10px", borderRadius:3, width:"fit-content" }}>{ex.tag}</span>
      <span style={{ fontFamily:SANS, fontWeight:700, fontSize:16, lineHeight:1.25, color:"#1a0a2e" }}>{ex.title}</span>
      {open && <p style={{ fontFamily:SANS, fontSize:13, fontWeight:400, lineHeight:1.75, color:"#1a0a2e", opacity:0.75, margin:0 }}>{ex.desc}</p>}
      <span style={{ fontFamily:MONO, fontSize:9, color:"#1a0a2e", opacity:0.35 }}>{open ? "tap to close" : "tap to read"}</span>
    </button>
  );
}
  ...
  return (
    <>
      <button onClick={() => setOpen(true)} style={{
        ...
        <span style={{ fontFamily:SANS, fontSize:14, fontWeight:500, color:"#1a0a2e", opacity:0.45 }}>{label}</span>
        <span style={{ fontFamily:MONO, fontSize:9, color:"#1a0a2e", opacity:0.25 }}>click to expand</span>
      </button>
      {open && (
  ...
  return (
    <main style={{ background:"#F7F3EC", color:"#1a0a2e", opacity:entered?1:0, transition:"opacity 0.55s ease" }}>
      ...
      <section style={{ padding:"80px clamp(16px,4vw,60px)" }} className="btc-dotgrid">
        <SectionTag text="Side Quests" bg="#1a0a2e" color="#E8DCC8" tilt="1deg" />
        <p style={{ fontFamily:SANS, fontSize:14, color:"#1a0a2e", opacity:0.5, maxWidth:480, lineHeight:1.75, marginBottom:36 }}>
      ...
      <section style={{ padding:"80px clamp(16px,4vw,60px)", background:"#E8DCC8" }} className="btc-kraft">
        ...
        <p style={{ fontFamily:MONO, fontSize:10, color:"#1a0a2e", opacity:0.35, marginTop:20 }}>
